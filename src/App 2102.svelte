...
<script>
	...
	const sketch = (p5) => {
		...
		let sineX = Math.sin(h1) * amp;
		let cosY = Math.cos(h1) * amp;
		...
		function ellps (radx,rady,num,radoffsetX,radoffsetY,ellprot ) {
			...
			let nrady = rady;
			  ...
			  for (let i = 0; i < num; i++) {
				  p5.stroke(p5.lerpColor(col1,col2,(i/num)));
				  p5.push();
...
<div class="controls">
	...
	<div class="row">
		<Range bind:value={h2} min={1} max={254} />
		<Range bind:value={s2} min={1} max={254} />
		<Range bind:value={b2} min={1} max={254} />
	</div>
	<div class="row">
		<Range bind:value={h1} min={1} max={254} />
