	  ...
	  function getRandomColor() {
		  ...
		  for (var i = 0; i < 6; i++) {
			...
			const randomize = () => {
				...
				ellipses = Math.random()*10;
				ellipseX = Math.random()*200;
				ellipseY = Math.random()*200;
				offsetX = Math.random()*(300-100);
	...
	p5.draw = () => { 
		...
		function ellps (radx,rady,num,radoffsetX,radoffsetY,ellprot ) {
			...
			// const colorB = Math.floor(Math.random()*16777215).toString(16);
			  ...
			  for (let i = 0; i < num; i++) {
				  p5.stroke(p5.lerpColor(col,colorB,(i/num)));
				//   p5.stroke('white');
				  p5.push();
...
//   }
  ...
  <div class="controls">
	  <div class="row">
		  <p>Ellipses {quantity}</p>
			  <input type="range" bind:value={quantity} min="1" max="50" step="1" />
	  </div>
	  <div class="row">
		  <p>Repeat {ellipses}</p>
			  <input type="range" bind:value={ellipses} min="1" max="20" step="1" />
	  </div>
	  <div class="row">
		  <p>Radius X {ellipseX}</p>
			  <input type="range" class="slider" bind:value={ellipseX} min="100" max="600" step="1" />
	  </div>
	  <div class="row">
		  <p>Radius Y {ellipseY}</p>
			  <input type="range" class="slider" bind:value={ellipseY} min="1" max="300" step="1" />
	  </div>
	  <div class="row">
		  <p>Indent X {offsetX}</p>
			  <input type="range" class="slider" bind:value={offsetX} min="-100" max="300" step="1" />
	  </div>
	  <div class="row">
		  <p>Indent Y {offsetY}</p>
			  <input type="range" class="slider" bind:value={offsetY} min="-100" max="300" step="1" />
	  </div>
	  <!-- <div class="row">
		  <p>Opacity {opacity}</p>
